    ...
    value: function getAttribute() {
      ...
      return {
        ...
        promotionCode: function promotionCode() {
          return Cookie.get('pc');
        },
        utmSource: function utmSource(key) {
          return Cookie.getUtm(key);
        },
        // get the utm source
        utmMedium: function utmMedium(key) {
          return Cookie.getUtm(key);
        },
        // get the utm medium
        utmTerm: function utmTerm(key) {
          return Cookie.getUtm(key);
        },
        // get the utm term
        utmContent: function utmContent(key) {
          return Cookie.getUtm(key);
        },
        // get the utm concent
        utmCampaign: function utmCampaign(key) {
          return Cookie.getUtm(key);
...
Cookie.setUtms(); // save d,o,b,pc to cookies

var drummerId = Url.getParameterByName('d');
var offerId = Url.getParameterByName('o');
var buyerId = Url.getParameterByName('b');
var promotionCode = Url.getParameterByName('pc');
if (drummerId) Cookie.set('d', drummerId, 2 * 365 * 24 * 60);
if (offerId) Cookie.set('o', offerId, 2 * 365 * 24 * 60);
if (buyerId) Cookie.set('b', buyerId, 2 * 365 * 24 * 60);
if (promotionCode) Cookie.set('pc', promotionCode, 2 * 365 * 24 * 60); // process the queue and future incoming commands
...
}(window, document, window["dpix"], "dpix", " https://d3g5rjzybokn2h.cloudfront.net/pixel.gif", 1));
